 ...
 * [PetTestEnvironment]. Discovered via `META-INF/services`; the spec is irrelevant here
 * because the environment is process-wide, so any spec mounting
 * `@ApplyExtension(WirespecExtension::class)` resolves the same contexts.
 */
class ScenarioContextProvider : ContextProvider {
    override fun endpointContext(spec: Spec): WirespecTestContext = PetTestEnvironment.endpointContext
    override fun channelContext(spec: Spec): WirespecChannelContext = PetTestEnvironment.channelContext
}
